...
namespace ant::OpenGl::Error
{
    ...
    void ErrorFunc(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const void *userParam)
    {
        ...
        {
            CORE_ERROR("OpenGl error: id = {0}, type = {1}, source = {2}, message = {3}", id, GetTypeStr(type), GetSourceStr(source), message);
            CORE_BREAK();
        }
        else if (severity == GL_DEBUG_SEVERITY_NOTIFICATION)
        {
            CORE_INFO("OpenGl error: id = {0}, type = {1}, source = {2}, message = {3}", id, GetTypeStr(type), GetSourceStr(source), message);
        }
        else
        {
